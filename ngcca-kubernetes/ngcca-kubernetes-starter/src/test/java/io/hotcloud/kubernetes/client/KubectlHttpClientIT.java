...
public class KubectlHttpClientIT extends ClientIntegrationTestBase {
    ...
    public void uploadFileToPod_then_downloadDirectoryToLocally() throws InterruptedException {
        ...
        PodList readList = podHttpClient.readList(NAMESPACE, labelSelector);
        List<Pod> pods = readList.getItems();
        List<String> podNames = pods.stream()
                .map(e -> e.getMetadata().getName())
                .collect(Collectors.toList());

        Boolean uploaded = kubectlHttpClient.upload(NAMESPACE, podNames.get(0), null, "/home/yaolianhua/.profile", "/hotcloud/.profile", CopyAction.FILE);
        Assertions.assertTrue(uploaded);

        Boolean downloaded = kubectlHttpClient.download(NAMESPACE, podNames.get(0), null, "/hotcloud/config", "/home/yaolianhua/download_config", CopyAction.DIRECTORY);
    ...
    public void portForward() throws InterruptedException {
        ...
        Boolean result = kubectlHttpClient.portForward(NAMESPACE, podNames.get(0), null, 8080, 8078, null, null);
    ...
    public void eventsRead() throws InterruptedException {
        ...
        List<Event> events = kubectlHttpClient.events(NAMESPACE);
        ...
        for (Map.Entry<String, String> entry : nameMessages.entrySet()) {
            ...
            Event eventResult = kubectlHttpClient.events(NAMESPACE, entry.getKey());
    ...
    List<HasMetadata> apply() {
        ...
        List<HasMetadata> result = kubectlHttpClient.resourceListCreateOrReplace(null, YamlBody.of(yaml));
        return result;
    ...
    Boolean delete() {
        ...
        Boolean result = kubectlHttpClient.delete(null, YamlBody.of(yaml));
        return result;
