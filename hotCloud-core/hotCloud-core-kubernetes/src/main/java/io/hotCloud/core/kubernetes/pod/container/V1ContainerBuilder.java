...
public final class V1ContainerBuilder {
    ...
    public static V1Container build(Container container) {
        ...
        if (null != containerStartupProbe) {
            V1Probe startupProbe = V1ProbeBuilder.build(containerStartupProbe);
            v1Container.setStartupProbe(startupProbe);
        }

        Resources resources = container.getResources();
        if (null != resources) {
            V1ResourceRequirements v1ResourceRequirements = V1ResourceRequirementsBuilder.build(resources);
            v1Container.setResources(v1ResourceRequirements);
        }

        List<V1EnvVar> v1EnvVars = V1EnvVarsBuilder.build(container.getEnv());
        v1Container.setEnv(v1EnvVars);

        List<V1EnvFromSource> v1EnvFromSources = V1EnvFromSourceBuilder.build(container.getEnvFrom());
