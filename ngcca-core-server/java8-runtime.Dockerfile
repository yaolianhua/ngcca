...
ARG FILE_SERVER
RUN wget -q ${FILE_SERVER}/Downloads/apache-tomcat-9.0.73.tar.gz -O /tmp/apache-tomcat-9.0.73.tar.gz
RUN mkdir -p /home/admin/tomcat && tar -xf /tmp/apache-tomcat-9.0.73.tar.gz -C /home/admin/tomcat --strip-components 1
RUN rm /tmp/apache-tomcat-9.0.73.tar.gz && chmod +x $CATALINA_HOME/bin/*sh
